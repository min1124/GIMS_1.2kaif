...
$(function(){
    ...
    $('#example tbody').on('dblclick', 'tr', function () {//大客户table选中某行
        ...
        // if ($(this).parent('table').hasClass('k3')){
        //     alert('1')
        // }else{
    ...
    $('#create2').click(function(){//其它客户生成按钮 
        ...
        var k = 0
        // var qty2 = 0
        var ghdw = td.eq(3).text();
        var po_number = td.eq(4).text();
        for(var i = 0; i < td.length/14; i++){
            // id = td.eq(i*14+0).text() +" " +id
            fnumber = td.eq(i*14+8).find('input')//td.eq(i*14+8).find('.fnumber').val() +" " +fnumber
                ...
                if (ghdw == '现款销售'){
                    for (i=0;i<=rs.data1.length-1;i++){
                        ...
                        var row1 ='<tr>'+
                            ...
                            '<th><input type="text" value='+rs.data2[i].qty_request+' ></th>'+
