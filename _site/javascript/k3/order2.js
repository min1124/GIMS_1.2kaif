...
$(function(){
    ...
    $('#create2').click(function(){//其它客户生成按钮 
        ...
        for(var i = 0; i < td.length/18; i++){
            ...
            var num2 = 0
            fnumber = td.eq(i*18+8).find('input')//td.eq(i*14+8).find('.fnumber').val() +" " +fnumber
            for(var j = 0; j < fnumber.length/3; j++){
                if(fnumber[0+j*3].checked==true){
                    id[k] = td.eq(i*14+0).text() 
                    k3fn[k] = fnumber[1+j*3].value
                    num[k] = (fnumber[2+j*3].value)
                    qty[k] = td.eq(i*18+11).text()
                    k++
                    qty2 = parseInt(td.eq(i*18+11).text())
    ...
    $('#create').click(function(){//大客户生成按钮  
        ...
        for(var i = 0; i < td.length/15; i++) {
            ...
            var num2 = 0
            fnumber = td.eq(i*15+7).find('input')
            for(var j = 0; j < fnumber.length/3; j++){
                if(fnumber[0+j*3].checked==true){
                    id[k] = td.eq(i*15+0).text() 
                    k3fn[k] = (fnumber[1+j*3].value)
                    num[k] = (fnumber[2+j*3].value)
                    qty[k] = td.eq(i*15+10).text()
                    k++
                    qty2 = parseInt(td.eq(i*15+10).text())
                    var num1 = parseInt(fnumber[2+j*3].value)
                    num2 = num2 + num1
            ...
            success: function(rs){
                ...
                for (i=0;i<=rs.data1.length-1;i++){
                    ...
                    var row4 ='<tr>'+
                          ...
                          '</tr>'
                    if(type=='zx'){
                        if(rs.data2[i].business_mode=='VCI-VRN'||rs.data2[i].business_mode=='VCI-PO'||rs.data2[i].business_mode=='HUB补货'){
    ...
    $('#other_save').click(function(){//其它订单保存
        ...
        for (var i = 0;i<inputs.length/6;i++){
            po_line_num[i] = inputs[0+i*6].value
            customer_item[i] = inputs[1+i*6].value
            supplier_item[i] = inputs[2+i*6].value
            vendor_name[i] = inputs[3+i*6].value
            qty_request[i] = inputs[4+i*6].value
            request_date[i] = inputs[5+i*6].value
        }
...
function initComplete(){ //大客户订单初始化
    ...
    var dataPlugin2 ='<div id="time" class=" pull-left dateRange" style="margin-left:200px;"> '+
            ...
            '</div>';
    $('#example_filter').append(dataPlugin1);
...
function initComplete1(){//其它订单初始化
    ...
    $('#example1_wrapper').children('.clear').append(dataPlugin1);
