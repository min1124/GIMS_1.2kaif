...
$(function () {
  ...
  $(document).on("dblclick","#example tbody tr",function() {//双击事件
    ...
    $.ajax({  
      'url': ip+'rejectyf/t_Rejection?sql='+a,   
      'type': 'post',
      'data': data,
      "error" : function(rs) {
        if(rs.status==401){
      ...
      'success': function(rs){
        $('#exampleYf').DataTable({
          ...
          "autoWidth": false,
          'ajax': {  
            'url': ip+'rejectyf/t_RejectionEntry?sql='+a,  
            'data': data,  
            'type': 'POST',
            "error" : function(rs) {
              if(rs.status==401){
        ...
        if(rs.data2.length>0){
          ...
          $("#statusYf").val(rs.data2[0].Status)//
  ...
  $(document).on("click","#shYf",function(){//审核按钮--研发
    if(confirm("确定通过吗？")){
      var bfje = $('#bfjeYf').val()
      var status=$('#statusYf').val();
      if (status==0){
        var data={
          name:name,
          token:token,
  ...
  $(document).on("click","#gbYf",function(){//终止按钮--研发
    if(confirm("确定终止吗？")){
      var status=$('#statusYf').val();
      if (status==0){
        alert("您没有终止权限！");
